...
When a feature needs a macOS permission such as Accessibility, Screen Recording or Microphone, that access is used only for the feature it belongs to. Captured content leaves the Mac only when you explicitly create a temporary link or upload it to a server you configured. The [permissions guide](PERMISSIONS.md) breaks down each permission.
